...
var findSubstring = function(s, words) {
  ...
  for (index = 0; index < words.length; index++) {
    keyMap[words[index]] = keyMap.hasOwnProperty(words[index])
      ? keyMap[words[index]] + 1
      : 1;
  }

  outerloop: for (index = 0; index <= boundary; index++) {
    let resultMap = {};

    for (let n = 0; n < words.length; n++) {
      const key = s.substring(
        index + wordLen * n,
        index + wordLen * n + wordLen,
      );
