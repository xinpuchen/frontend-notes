 ...
 *
 * 基本概念演示：https://user-gold-cdn.xitu.io/2018/4/16/162cd23e69ca9ea3?imageslim
 *
 * 时间复杂度：最好：nlogn，平均：nlogn，最差：n2
 *
 * 辅助空间：log(n)
 *
 * 稳定性：不稳定
 *
 * 时间复杂度：因为一个单独的快速排序调用牵涉了O(n)的工作，加上对n/2大小之数列的两个递归调用，这个关系式可以是：
 * T(n) = O(n) + 2T(n/2)
 *
 * 解决这种关系式类型的标准数学归纳法技巧告诉我们 T(n) = O(nlogn)
 * 最坏情况：两子数列拥有大各为 1 和 n-1，且调用树（call tree）变成为一个 n个嵌套（nested）调用的线性连串（chain）
 *
 * 空间复杂度：因为最好的情况最多需要 O(log n) 次的嵌套递归调用，所以它需要 O(log n) 的空间。最坏情况下需要 O(n) 次嵌套递归调用，因此需要 O(n) 的空间。
 *
 * 改进方法
 * 1、随机选取基准元素
 * 2、当排序数目分割到一定大小后，使用插入排序（处理有序或重复数字）
 * 3、三路快排（equal-less-...-greater-equal）
 */
